...
void ctp_quote::init()
{
    ...
    strncpy(req->BrokerID,const_cast<char*>(simu_cfg.get_para("BROKER_ID").c_str()),sizeof(req->BrokerID));
    strncpy(req->UserID,const_cast<char*>(simu_cfg.get_para("INVESTOR_ID").c_str()),sizeof(req->UserID));
    strncpy(req->Password,const_cast<char*>(simu_cfg.get_para("PASSWORD").c_str()),sizeof(req->Password));


	ppInstrumentID=new char * [MAX_CONTRACT_NUMBER];
    list<string> contracts=wfunction::splitstring(simu_cfg.get_para("INSTRUMENT_ID"));
...
void ctp_quote::login()
{
    ...
    pUserApi->RegisterFront(const_cast<char*>(simu_cfg.get_para("QUOTE_FRONT_ADDR").c_str()));
	pUserApi->Init();
	pUserApi->Join();
}
...
char *ctp_quote::mk_quote_con_dir()
{
    ...
    string exedir=simu_cfg.get_para("QUOTE_CON_PATH");
    if(exedir.size()>0)
    {
