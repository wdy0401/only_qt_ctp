...
std::string ctp_order_manager::new_order(const std::string InstrumentID,const std::string side, const std::string & openclose ,double price,long size)
{
    ...
    strncpy(ctporder->of->BrokerID,const_cast<char*>(simu_cfg.get_para("BROKER_ID").c_str()),sizeof(ctporder->of->BrokerID));
    strncpy(ctporder->of->UserID,const_cast<char*>(simu_cfg.get_para("INVESTOR_ID").c_str()),sizeof(ctporder->of->UserID));
...
CThostFtdcInputOrderField * ctp_order_manager::initorder(const string & InstrumentID, const string & side, const string & openclose, double price, long size)
{
    ...
    ///经纪公司代码
    strncpy(oireq->BrokerID, const_cast<char*>(simu_cfg.get_para("BROKER_ID").c_str()), sizeof(oireq->BrokerID));
    ///投资者代码
    strncpy(oireq->InvestorID, const_cast<char*>(simu_cfg.get_para("INVESTOR_ID").c_str()), sizeof(oireq->InvestorID));
    ///合约代码
    ...
    ///用户代码
    strncpy(oireq->UserID, const_cast<char*>(simu_cfg.get_para("INVESTOR_ID").c_str()), sizeof(oireq->UserID));
    ///报单价格条件
